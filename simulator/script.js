...
let lining = null;
let bg;
let localStorageTypesKey = "circuitsim_types",
  localStorageOperationsKey = "circuitsim_operations",
  localStorageNodeKey = "circuitsim_node";
  ...
  if (localtypes) {
    ...
    extraKeys.forEach((key) => {
      let groupNode = new GROUP(
        key,
        nodeData[types[key]].inputNames,
        nodeData[types[key]].outputNames,
        operations[types[key]]
      );
...
function keyPressed() {
  if (key == "Delete") {
    if (
      !isInsideRect(nearest.x, nearest.y, nearest.w, nearest.h, mouseX, mouseY)
    )
      return;
    optionNode = nearest;
...
function mousePressed() {
  ...
  for (let i = 0; i < nearest.inputPos.length; i++) {
    ...
    if (
      isInsideRect(nearest.x + p.x, nearest.y + p.y, 12, 12, mouseX, mouseY)
    ) {
      lining = {
        node: nearest,
        index: i,
  ...
  if (lining?.type == "output") {
    for (let i = 0; i < nearest.inputPos.length; i++) {
      ...
      if (
        ...
        if (
          nearest.type == types.negedgepulse ||
          nearest.type == types.posedgepulse
        )
    ...
    for (let i = 0; i < nearest.outputPos.length; i++) {
      ...
      if (
        ...
        if (nearest.type == types.junction) {
          ...
          lining.node.type == types.negedgepulse ||
          lining.node.type == types.posedgepulse
        ) {
...
function renameNode(node) {
  ...
  if (node.type == types.input || node.type == types.output) {
    ...
    if (!name.match(/^\w+$/)) {
      alert(
        "Only letters numbers and underscores, must not start with a number"
      );
      return;
    }
    node.name = name;
...
function createGroup() {
  try {
    if (!nodes) return;
    let inputs = nodes.filter(
      (node) => node.type == types.input || node.type == types.pulse
    );
    let gNodes = [];
    let _ids = [];
    ...
    let duplicateInps = inpNames.filter(
      (inp, i) => inpNames.indexOf(inp) !== i
    );
    let duplicateOutps = outNames.filter(
      (out, i) => outNames.indexOf(out) !== i
    );
    if (duplicateInps.length > 0 || duplicateOutps.length > 0) {
      alert(
        "All inputs and outputs must be named unique, Hover and press CTRL+A to rename"
      );
      return;
    }
...
function DFS(node, _ids = []) {
  ...
  for (let i = 0; i < node.outputNodes.length; i++) {
    ...
    if (node.outputNodes[i]?.node.type == types.output) {
      node.outputNodes[i].node.name =
        node.outputNodes[i].node.name || node.outputNodes[i].node.type;
      _ids.push(node.outputNodes[i].node.id);
      _nodes.push(node.outputNodes[i].node);
    } else _nodes.push(...DFS(node.outputNodes[i]?.node, _ids));
    if (!node.outputNodes[i].node.inputNodes)
      node.outputNodes[i].node.inputNodes = [];
    let check = node.outputNodes[i].node.inputNodes.filter(
      (n) => n.node.id == node.id && n.index == i
    );
    if (check.length == 0)
...
function expandCanvas() {
  ...
  if (width > windowWidth) {
    let yes = confirm(
      "All progress will be lost, Smaller canvas will run faster on mobile devices. Want to contract?"
    );
    if (yes)
    ...
    let yes = confirm(
      "All progress will be lost, On mobile devices expanding window can slow down this simulator. Want to expand?"
    );
...
function clearAllData() {
  if (confirm("All Data will be lost... Continue?")) {
    if (
      confirm("All Group Nodes will be deleted for ever! Want to delete????")
    ) {
      localStorage.clear();
...
var handleEvent = function (e) {
  if (moving) e.preventDefault(); // disables scrolling by mouse wheel and touch move
};
